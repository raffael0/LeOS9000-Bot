...
public class GlobalChatCMD extends Command {
    ...
    protected void execute(CommandEvent event) {
        if (CubingCMDUtil.getStatus(event.getGuild().getId()).equals("off") && event.getChannel().getName().equals("global") && !GlobalChatUtil.banned(event.getGuild().getId())) {
        ...
        } else if (GlobalChatUtil.banned(event.getGuild().getId())) {
            event.reply("Error, you cannot connect to the global chat because you were kicked earlier! Please try again later!");
        }
    }
}
